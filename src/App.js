...
export default function App() { 
  ...
  const calcTimes = (extraOn, newUsers) => { 
    ...
    const right = RIGHT_TOTAL; const baseLeft = 220; 
    const baseRight = 240; const baseNew = 80; 
    ...
    const meanWithout = (LEFT_TOTAL * (baseLeft + 2 * (LEFT_TOTAL / 100)) + right * (baseRight + 2 * (right / 100))) / (LEFT_TOTAL + right); 
    const totalVehicles = LEFT_TOTAL + right; 
    const sumTimesWith = leftRemaining * timeLeft + leftUsingNew * timeNew + right * timeRight; 
    const meanWith = sumTimesWith / totalVehicles; 
    return { timeLeft: Number(timeLeft.toFixed(1)), timeRight: Number(timeRight.toFixed(1)), timeNew: Number(timeNew.toFixed(1)), meanWithout: Number(meanWithout.toFixed(1)), meanWith: Number(meanWith.toFixed(1)), }; };
